...
function jinyun_print($order, $store,$print)
{
    ...
    $orderinfo .= "<S3><AM>{$store['title']}\n</AM></S3>";
    if($order['order_type'] != 2){
        $orderinfo .= "<S1>桌号：{$order['table_info']}\n</S1>";
    }

    $orderinfo .= "<S1>名称　　　　　 单价  数量   金额\n</S1>";
    $orderinfo .= "<S1>--------------------------------\n</S1>";
    if (!empty($order['dish'])) {
    ...
    $orderinfo .= "<S1>--------------------------------\n</S1>";
    $orderinfo .= "<S3>合计：{$order['price']}元\n</S3>";
    if (!empty($order['note'])) {
        $orderinfo .= "<S1>备注：" . $order['note'] . "\n</S1>";
    }

    if ($order['is_usecard'] == 1) {
        $price = $order['price'] + $order['delivery_fee'] - $order['card_fee'];
        $orderinfo .= "<S1>使用优惠券: -{$price}元\n</S1>";
        $orderinfo .= "<S1>实际支付: {$order['card_fee']}元\n</S1>";
    }
    if ($order['order_type'] == 1) {
        $orderinfo .= "<S1>下单人：{$order['username']}\n</S1>";
        $orderinfo .= "<S1>支付方式：{$order['pay_type']}\n</S1>";
        $orderinfo .= "<S1>联系电话：{$order['mobile']}\n</S1>";
        $orderinfo .= "<S1>就餐人数：{$order['person_num']}人\n</S1>";
        $orderinfo .= "<S1>订单号：{$orderid}\n</S1>";

    } elseif ($order['order_type'] == 2) {
        $orderinfo .= "<S1>送餐地址：{$order['address']}\n</S1>";
        $orderinfo .= "<S1>下单人：{$order['username']}\n</S1>";
        $orderinfo .= "<S1>联系电话：{$order['mobile']}\n</S1>";
        $orderinfo .= "<S1>配送费：{$order['delivery_fee']}\n</S1>";
        $orderinfo .= "<S1>支付方式：{$order['pay_type']}\n</S1>";
        $orderinfo .= "<S1>送餐时间：{$order['delivery_time']}\n</S1>";
    } else {
        $orderinfo .= "<S1>下单人：{$order['username']}\n</S1>";
        $orderinfo .= "<S1>支付方式：{$order['pay_type']}\n</S1>";
        $orderinfo .= "<S1>联系电话：{$order['mobile']}\n</S1>";
