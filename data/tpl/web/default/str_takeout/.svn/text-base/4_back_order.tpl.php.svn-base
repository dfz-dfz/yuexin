					...
					<form class="form-horizontal form" id="form1" action="<?php  echo $this->createWebUrl('manage', array('op' => 'back_submit'));?>" method="post" enctype="multipart/form-data">
						...
						<table class="table table-hover table-bordered">
							...
							<tr id="type1">
								<th colspan="2">
									<select class="form-control" name="table_id">
										...
										<?php  if(is_array($tables)) { foreach($tables as $table) { ?>
											<option value="<?php  echo $table['id'];?>"><?php  echo $table['title'];?></option>
										<?php  } } ?>
									</select>
								</th>
							</tr>
							<tr id="type2" style="display:none">
								<th colspan="2">
									<input type="text" name="username" value="<?php  echo $member['username'];?>" class="form-control" style="margin-bottom:10px;" placeholder="下单人">
									<input type="text" name="mobile" value="<?php  echo $member['mobile'];?>" class="form-control" style="margin-bottom:10px;" placeholder="手机号">
									<input type="text" name="address" value="<?php  echo $member['address'];?>" class="form-control" placeholder="送餐都地址">
								</th>
							</tr>
							<tr>
								<th colspan="2">
				...
				<form class="form-horizontal form" id="form2" action="" method="post" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<div class="col-sm-8 table-responsive">
								...
								<tbody>
									<?php  if(is_array($members)) { foreach($members as $member) { ?>
										<tr>
											...
											<td>
												<?php  echo $groups[$member['groupid']]['title'];?> 	
											</td>
											<td>
							...
							<?php  } ?>
							<?php  if($uid > 0 && !empty($member)) { ?>
								<p class="form-control-static text-danger" id="user-info" data-uid="<?php  echo $uid;?>">
								UID：<?php  echo $member['uid'];?> 
								<?php  if(!empty($member['groupid'])) { ?>
								 会员组：<?php  echo $groups[$member['groupid']]['title'];?> 
								<?php  } ?>
								<?php  if(!empty($member['realname'])) { ?>
								 姓名：<?php  echo $member['realname'];?> 
								<?php  } ?>
								<?php  if(!empty($member['mobile'])) { ?>
								 手机号：<?php  echo $member['mobile'];?> 
								<?php  } ?>
								<?php  if(!empty($member['nickname'])) { ?>
								 微信昵称：<?php  echo $member['nickname'];?> 
								<?php  } ?>
								</p>
							<?php  } ?>
						</div>
		...
		<div class="pull-left col-lg-12" id="dish">
			<div class="panel panel-default" style="padding:1em">
					...
					<div class="container-fluid">
						...
						<ul class="nav navbar-nav nav-btns" id="cate">
							...
							<?php  if(is_array($categorys)) { foreach($categorys as $category) { ?>
								<li data-id="<?php  echo $category['id'];?>"><a href="javascript:;"><?php  echo $category['title'];?></a></li>
							<?php  } } ?>
						</ul>
					</div>
				</nav>
				<div class="panel-body">
					<?php  if(is_array($categorys)) { foreach($categorys as $category) { ?>
						<div id="cate-<?php  echo $category['id'];?>" class="cate-dish">
							<?php  if(is_array($data[$category['id']])) { foreach($data[$category['id']] as $row) { ?>
								<a href="javascript:;" class="tile img-rounded" id="dish-right-<?php  echo $row['id'];?>" data-id="<?php  echo $row['id'];?>" data-price="<?php  echo $row['price'];?>">
									<strong><?php  echo $row['price'];?> 元/份</strong>
...
require(['util'], function(util){
	var dishes = '<?php  echo $dishes_str;?>';
